...
from mapping.config import PROBLEMS_CONFIG
...
source_data, target_data = problem_factory(source_name=args.source,target_name=args.target)
...
################# TRAINING TRANSITION MODEL and DISCRIMINATOR ##############
...
model_names = [
    "Trained on "+str(H)+" target",
    "Trained on "+str(len(data_formatter_t_val.as_transitions()))+" target",
    "Trained on "+str(H)+" target + "+str(len(data_formatter_s.as_transitions()))+" synthetic",
    "Trained on "+str(len(data_formatter_s.as_transitions()))+" synthetic",
]
errors = []
...
labels = [
    str(H)+" target",
    str(len(data_formatter_t_val.as_transitions()))+" target",
    str(H)+" target + \n"+str(len(data_formatter_s.as_transitions()))+" synthetic",
    str(len(data_formatter_s.as_transitions()))+" synthetic",
]
